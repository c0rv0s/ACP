...
contract MockHookAdapter {
    ...
    function setCanonicalPoolKey(PoolKey memory key) external {
        _poolKey = key;
    }
    ...
    function previewMetadata(address, bytes calldata) external pure returns (AGCDataTypes.HookMetadata memory metadata) {
        metadata.qualityScoreBps = uint16(AGCDataTypes.BPS);
    }

    function setNextSnapshot(AGCDataTypes.EpochSnapshot memory snapshot) external {
    ...
    function setRewardReceipt(uint256 receiptId, AGCDataTypes.RewardReceipt memory receipt) external {
    ...
    function consumeRewardReceipt(uint256 receiptId) external returns (AGCDataTypes.RewardReceipt memory receipt) {
        receipt = receipts[receiptId];
        receipt.consumed = true;
        receipts[receiptId].consumed = true;
    }

    function setController(address) external {}
    function setRewardDistributor(address) external {}
    function setTrustedRouter(address router, bool trusted) external { trustedRouters[router] = trusted; }
    function setRegime(AGCDataTypes.Regime newRegime) external { currentRegime = newRegime; }
    function setFeeConfig(AGCDataTypes.HookFeeConfig calldata config) external { _feeConfig = config; }
}
